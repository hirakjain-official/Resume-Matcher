...
An AI-powered resume matching system that analyzes and scores resumes against job descriptions
using advanced AI models including Claude, GPT, and DeepSeek.
"""
...
import time, requests, statistics, base64, os, markdown, io
...
DEEPSEEK_CONTEXT_WINDOW = int(os.getenv('DEEPSEEK_CONTEXT_WINDOW'))
...
# Initialize the OpenRouter client globally for DeepSeek
default_openrouter_client = openai.OpenAI(
    api_key=os.getenv('OPENROUTER_API_KEY'),
    base_url="https://openrouter.ai/api/v1"
)

# Global variable to store the chosen API (using OpenRouter with DeepSeek)
chosen_api = "openrouter"
...
def choose_api():
    global chosen_api
    chosen_api = "openrouter"  # Always use OpenRouter with DeepSeek
    print(colored(f"\nUsing OpenRouter DeepSeek API", "green", attrs=["bold"]))
...
def talk_to_ai(prompt,
               ...
               client=None):
    try:
        response = talk_to_openrouter(prompt, max_tokens, image_data, client)
        return response.strip() if response else ""
    except Exception as e:
        logging.error(f"Error in talk_to_ai: {str(e)}")
        return ""

# Removed talk_to_anthropic function - using only OpenRouter DeepSeek

# Removed talk_to_openai function - using only OpenRouter DeepSeek

def talk_to_openrouter(prompt,
                      max_tokens=DEFAULT_MAX_TOKENS,
                      image_data=None,
                      client=None):
    if client is None:
    ...
    # OpenRouter with DeepSeek currently supports text-only, so we'll handle text
    try:
        response = client.chat.completions.create(
            ...
            extra_headers={
                "HTTP-Referer": "https://github.com/resume-matcher",
                "X-Title": "Resume Job Matcher"
            }
        )
        return response.choices[0].message.content.strip()
    except Exception as e:
        logging.error(f"Error in OpenRouter DeepSeek communication: {str(e)}")
        return ""
...
def talk_fast(messages,
              ...
              client=None,
              image_data=None):
    # Always use OpenRouter with DeepSeek
    if model is None:
    ...
    # Estimate token count for DeepSeek (roughly 4 chars per token)
    ...
    if input_tokens + max_tokens > context_window:
        max_tokens = context_window - input_tokens - 100  # Reserve tokens for safety
        if max_tokens <= 0:
...
def match_resume_to_job(resume_text, job_desc, file_path, resume_images, client=None):
    ...
    for criterion in criteria:
        ...
        try:
            ...
            # Debug logging
            logging.info(f"Criterion '{criterion['name']}': Raw response='{response}', Parsed score={score}")
    ...
    if isinstance(reasons_response, dict) and 'content' in reasons_response:
        ...
        match_reasons = reasons_response.strip()
    else:
    ...
    elif isinstance(website_response, str):
        website = website_response.strip()
    else:
...
def get_score_details(score):
    ...
    score_ranges = [
        {"min_score": 100, "max_score": 101, "label": 'Legendary Unicorn',      "color": 'magenta',     "emoji": '🦄'},
        {"min_score": 99, "max_score": 100, "label": 'Dream Candidate',        "color": 'yellow',      "emoji": '🏆'},
        ...
        {"min_score": 45, "max_score": 50,  "label": 'Unqualified Candidate',  "color": 'yellow',      "emoji": '🍊'},
        {"min_score": 40, "max_score": 45,  "label": 'Mismatched Skills',      "color": 'yellow',      "emoji": '🥕'},
        {"min_score": 35, "max_score": 40,  "label": 'Inadequate Fit',         "color": 'yellow',      "emoji": '🦊'},
...
def unify_format(extracted_data):
...
You can only speak in clean, concise, Markdown format.     
    ...
    # PDF generation removed - only saving as Markdown
...
def worker(args):
    ...
    try:
        ...
        unified_resume, resume_images = unify_format(extracted_data)
...
def process_resumes(job_desc, pdf_files):
    max_workers = min(4, len(pdf_files))  # Limit workers to avoid resource issues
    executor = None
    
    try:
        executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=max_workers,
            thread_name_prefix='ResumeProcessor'
        )
        
        # First, parallelize the unification process
        unified_futures = {executor.submit(unify_single_resume, file): file for file in pdf_files}
        
        unified_results = {}
        with tqdm(total=len(pdf_files), desc="Unifying resumes", unit="file") as pbar:
            for future in concurrent.futures.as_completed(unified_futures, timeout=300):  # 5 min timeout
                file = unified_futures[future]
                try:
                    unified_resume, resume_images = future.result(timeout=30)  # 30s per resume
                    unified_results[file] = (unified_resume, resume_images)
                except Exception as e:
                    logging.error(f"Error unifying {file}: {str(e)}")
        ...
        with tqdm(total=len(pdf_files), desc="Matching resumes", unit="file") as pbar:
            for future in concurrent.futures.as_completed(match_futures, timeout=600):  # 10 min timeout
                file = match_futures[future]
                try:
                    result = future.result(timeout=60)  # 60s per match
                    results.append(result)
                except Exception as e:
                    basename = os.path.basename(file)[:20]
                    logging.error(f"Error matching {file}: {str(e)}")
        ...
        for file in pdf_files:
            basename = os.path.basename(file)[:20]
            results.append((basename, 0, "🔴", "red", f"Executor Error: {str(e)}", "", "", []))
    
    finally:
        if executor:
            try:
                # Gracefully shutdown executor - Python 3.9+ supports cancel_futures
                try:
...
def unify_single_resume(file):
    extracted_data = extract_text_and_image_from_pdf(file)
    return unify_format(extracted_data)
...
def match_single_resume(job_desc, file, unified_resume, resume_images):
    ...
    try:
        ...
        return (basename, 0, "🔴", "red", error_msg, "", "", [])

def process_single_resume(job_desc, file):
    basename = os.path.basename(file)[:20]
    try:
        extracted_data = extract_text_and_image_from_pdf(file)
        unified_resume, resume_images = unify_format(extracted_data)
    ...
    try:
        if isinstance(result, str):
            ...
            if not result or result.strip() == '':
                return (basename, 0, "🔴", "red", "Error: Empty response from API", "", "", [])
            result = json5.loads(result)
        ...
        error_msg = f"API response parsing error (likely insufficient credits)"
        return (basename, 0, "🔴", "red", error_msg, "", "", [])
    except Exception as e:
        error_msg = f"Unexpected error: {str(e)}"
        return (basename, 0, "🔴", "red", error_msg, "", "", [])
...
def main():
    ...
    # Removed font and PDF options for simplicity
    parser.add_argument("pdf_folder", nargs="?", default=r"C:\Users\user\projects\masterplan\Resumes-20250920T120828Z-1-001\Resumes", help="Folder containing PDF resumes")
    
    args = parser.parse_args()

    # Font styles removed for simplicity
    ...
    print("\n" + "="*60)
    print(colored("📋 RESUME JOB MATCHER", 'cyan', attrs=['bold']))
    print("="*60)
    
    # Get job description via CLI input
    print(colored("\nPlease enter the job description:", 'yellow', attrs=['bold']))
    print(colored("(Paste the job description and press Ctrl+Z then Enter on Windows, or Ctrl+D on Linux/Mac when done)", 'green'))
    print("-" * 60)
    ...
    if not job_desc:
        print(colored("Error: No job description provided!", 'red'))
        sys.exit(1)
        
    print(colored(f"\n✅ Job description received ({len(job_desc)} characters)", 'green'))
    
    # Skip job description analysis for now to focus on resume matching
    analyze_job_desc = False
